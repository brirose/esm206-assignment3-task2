...
sw_split_name <- starwars %>% # start with starwars
  separate(col = name, # take column 'name'
           into = c("name1", "name2"), # split in 2
           sep = "-") #split at '-'

# The name in Row 49 is truncated after the second dash
```
...
sw_skin_sep <- starwars %>% # start with starwars
  separate(col = skin_color, # seperate 'skin_color'
           into = c("skin_1", "skin_2"), # into 2 columns
           sep = ",", # split at comma
           extra = "merge", #merge all after first comma into second column
           remove = F) #keep initial 'skin_color'
...
mpg_model_class <- mpg %>% # start with mpg
  unite(col = "model_class", # make a new column "model_class"
        c("model", "class"), # that combines "model" and "class"
        sep = "-", # seperate with a "-"
        remove = F) # keep original attributes
...
```{r}
# Your code here: 
ymd("2020-10-30") #YYYY-MM-DD to YYYY-MM-DD

ymd("2020/10/30") #YYYY/MM/DD to YYYY-MM-DD

ymd("20201030") #YYYYMMDD to YYYY-MM-DD
```
...
```{r}
# Your code here: 
some_dates <- c(11061972, 02142007, 01301843) #create vector
some_dates_converted <- mdy(some_dates) # convert dates to YYYY-MM-DD

class(some_dates_converted) #check class
...
gasol_hookshots <- lakers %>%  # start with lakers
  filter(player == "Pau Gasol" , #filter for Pau and hookshot
         type == "hook") %>% 
  mutate(date_new = ymd(date), # make new column with re-formatted date, classified "Date"
         month_new = month(date_new), # make new column for month
         year_new = year(date_new)) %>% # make new column for year
  group_by(month_new) %>% # do not need to filter further because already the desired season
  count(month_new) # create count table
...
```{r}
# Your code here: 
storms_danger <- storms %>% # start with storms
  mutate(danger = case_when( # add new column "danger"
    status == "hurricane" ~ "high", # danger when status is high
    TRUE ~ "moderate" # danger at other status
  ))
